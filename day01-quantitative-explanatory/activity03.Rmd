...
```{r load-data}
hfi <- read_tsv("https://www.openintro.org/data/tab-delimited/hfi.txt")
...
ggplot(hfi_2016, aes(x = pf_expression_control)) +
 ...
 # Histogram for pf_expression_control
ggplot(hfi_2016, aes(x = pf_expression_influence)) +
  geom_histogram(binwidth = 0.25, fill = "skyblue", color = "black") +
  labs(title = "Distribution of Personal Freedom Expression Influence in 2016",
       x = "PF Expression Internet",
       y = "Frequency",
       caption = "Source: Open Intro Human Freedom Index https://www.openintro.org/data/index.php?data=hfi") +
  theme_minimal()
...
```{r pairs-plot}
hfi_2016 %>% 
  select(pf_score, pf_expression_control, pf_expression_influence) %>% 
  ggpairs()
```
...
mlr_mod <- lm_spec %>% 
fit(pf_score ~ pf_expression_control + pf_expression_influence, data = hfi_2016)
...
```{r 3dscatterplot}
scatterplot3d(hfi_2016$pf_score, hfi_2016$pf_expression_control, hfi_2016$pf_expression_influence)
```
